...
use color_eyre::eyre::{Context, Ok, bail};
...
impl AppState {
    ...
    pub fn draw_ratatui_terminal_draw(&self, ratatui_frame: &mut ratatui::Frame) {
    ...
    ///
    /// }
    pub fn handle_events(&mut self) -> color_eyre::eyre::Result<()> {
        match crossterm::event::read().expect("failed to read crossterm events") {
            crossterm::event::Event::Key(key_event) if key_event.kind == crossterm::event::KeyEventKind::Press => {
                self.handle_key_events(key_event).wrap_err_with(|| format!("handling key events faild:\n {key_event:#?}"))
            }
            _ => Ok(()),
        }
    }
    ...
    pub fn handle_key_events(&mut self, key_event: crossterm::event::KeyEvent) -> color_eyre::eyre::Result<()> {
